...
DISCORD_MESSAGE = re.compile(
    r"(?i)(?:https?://)?(?:canary\.|ptb\.|www\.)?discord(?:app)?.(?:com/channels|gg)/(?P<guild_id>[0-9]{17,22})/(?P<channel_id>[0-9]{17,22})/(?P<message_id>[0-9]{17,22})"
)
...
YOUTUBE_WILDCARD = re.compile(
    ...
    (?:\s*) 
    (?:https?:\/\/)? 
    (?:www\.)?(?:youtube\.com|youtu\.be|youtube-nocookie\.com) 
    (?:\/(?:watch\?v=|embed\/|v\/|shorts\/|e\/|live\/)?)? 
    ([\w-]{11}) 
    (?:[&?][\w=]*)* 
    (?:[#\/]?|$) 
    (?:\s*) 
